...
async function loadTemplate(code) {
  loading.value = true
  try {
    const res = await fetch(`http://localhost:8080/api/admin/template?code=${code}`)
...
async function saveTemplate() {
  try {
    ...
    const res = await fetch(`http://localhost:8080/api/admin/template/update/${route.params.code}`, {
      method: 'PUT',
